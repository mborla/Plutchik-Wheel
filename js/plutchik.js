function plutchik(id, json){
    ...
    d3.json(json, function(data) {
         ...
         label.append("text")
               .attr("transform", function(d) { return (x(d.emotion) + x.bandwidth() / 2 + Math.PI / 2) % (2 * Math.PI) < Math.PI ? "rotate(90)translate(0,-20)" : "rotate(-90)translate(0,27)"; })
               .attr("id", function(d) { return "text_total_" + d.emotion; })
         ...
         label.append("text")
              ...
              .text(function(d) { return d.emotion + " " + d.degree_1; })
              .style("font-size",14)
         ...
         label.append("text")
              ...
              .text(function(d,i) { return emotions[0][i] + " " + d.degree_0; })
              .style("font-size", 14)
              .style("fill", "gainsboro")
              .style("visibility", "hidden"); 

         label.append("text")
              .attr("transform", function(d) { return (x(d.emotion) + x.bandwidth() / 2 + Math.PI / 2) % (2 * Math.PI) < Math.PI ? "rotate(90)translate(0,-37)" : "rotate(-90)translate(0,44)"; })
              .attr("id", function(d,i) { return "text_" + emotions[2][i]; })
              .text(function(d,i) { return emotions[2][i] + " " + d.degree_2; })
              .style("font-size", 14)
              .style("fill", "gainsboro")
              .style("visibility", "hidden");
         ...
         d3.selectAll(".arc")
           .on("mouseover", function(){
              ...
              d3.selectAll(".arc").style("fill", "gainsboro");
              ...
              strong = emotions[0][index];
              weak = emotions[2][index];            
              
              d3.selectAll("#text_total_"+principal).style("visibility", "hidden");
              d3.select("#text_"+strong).style("visibility", "visible");
              d3.select("#text_"+principal).style("visibility", "visible");
              d3.select("#text_"+weak).style("visibility", "visible");

              d3.select("#text_"+name).style("font-weight", "bold").style("fill", "black");
              
         })
         .on("mouseout", function(){

              d3.selectAll(".arc").style("fill", function(d,i) { var degree = d3.select(this.parentNode).attr("value"); return z[degree][i]; })
              ...
              d3.select("#text_"+strong).style("visibility", "hidden");
              d3.select("#text_"+principal).style("visibility", "hidden");
              d3.select("#text_"+weak).style("visibility", "hidden");

              d3.select("#text_"+name).style("font-weight", "400").style("fill", "gainsboro");
         })
